const commentService = require('../services/commentService');
  ...
  const getCommentForUser = async (req, res) => {
    ...
    try {
      const comments = await commentService.getCommentForUser(userId);
  ...
  if (
    ...
    res.status(400).send({
      ...
      data: {
        error:
          "One of the following keys is missing or is empty in request body: 'comment_content', 'user_id', 'post_id'",
      },
    });
    return;
  }
